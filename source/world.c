...
// 		uint8_t * const command = bullshitcore_memory_retrieve(sizeof "dd if= skip= count=B bs= | zcat -" - 1 + strlen(region_file_name) + LOG10(region_offset) + 2 + LOG10(chunk_size) + 1 + sizeof CHUNK_READ_BLOCK_SIZE - 1);
